 ...
 * The active palette.
 *
 * Falls back to the default theme rather than throwing when no provider is
 * above it: `earth` is an exact extraction of the palette the app was already
 * hardcoding, so a component rendered outside the provider — in a test, or
 * behind a boundary that lost it — shows the correct colours instead of
 * crashing the page. A missing provider is still a wiring mistake, so it is
 * reported in development.
 *
 * useThemeControls still throws: a theme switcher that silently does nothing
 * would be a real bug, not a graceful degradation.
 */
export function useTheme(): Theme {
  const ctx = useContext(ThemeContext)
  if (!ctx) {
    if (import.meta.env.DEV) {
      console.warn('useTheme: no ThemeProvider above this component — falling back to the default theme')
    }
    return THEMES.earth
  }
  return ctx.theme
