  ...
  return (
    <div className="border border-gray-200 rounded-xl p-4 space-y-3">
      ...
      {showForm && (
        <div className="bg-gray-50 rounded-lg p-3 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">ת.ז</label>
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">תאריך לידה</label>
              <input className={inputClass} type="date" dir="ltr" value={form.birth_date} onChange={e => setForm({ ...form, birth_date: e.target.value })} />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">סוג העסקה</label>
              <select className={`${inputClass} bg-white`} value={form.employment_type}
                onChange={e => setForm({ ...form, employment_type: e.target.value as EmploymentType })}>
                {EMPLOYMENT_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">הכנסה חודשית (₪)</label>
      ...
      {borrowers.length === 0 && !showForm && (
        <p className="text-center text-sm text-gray-400 py-3">אין לווים נוספים או ערבים</p>
      )}
