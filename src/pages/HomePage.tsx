          ...
          <div className="md:w-1/2 p-10">
            <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
              <div>
                ...
                <input type="text" value={contactForm.name} onChange={(e) => setContactForm((p) => ({ ...p, name: e.target.value }))} className="editorial-input w-full py-3 text-base" placeholder="ישראל ישראלי" />
              </div>
              <div>
                <label className="block text-xs font-bold text-primary uppercase tracking-wider mb-2">טלפון</label>
                <input type="tel" value={contactForm.phone} onChange={(e) => setContactForm((p) => ({ ...p, phone: e.target.value }))} className="editorial-input w-full py-3 text-base" placeholder="050-0000000" />
              </div>
              <div>
              ...
              <button type="submit" className="w-full py-4 bg-brand-gold text-white rounded-lg font-bold text-lg hover:opacity-90 transition-all active:scale-95">
                שליחת בקשה
              </button>
            </form>
          </div>
