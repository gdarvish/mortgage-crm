    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4" dir="rtl">
        <div className="bg-white rounded-2xl shadow-lg p-8 text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">לא ניתן לטעון את הפורטל</h1>
          <p className="text-gray-600">{errorMessage || 'הקישור אינו בתוקף. נא לפנות ליועץ המשכנתאות.'}</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <div className="max-w-2xl mx-auto p-4 -mt-4 space-y-4">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5">
          <h2 className="text-lg font-bold text-gray-900">שלום {data.first_name}!</h2>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5">
            ...
            {data.pending_signatures.map((sig, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg mb-2 last:mb-0">
                <span className="text-sm text-gray-800">{sig.document_name}</span>
                {sig.sign_url_token && (
                  <a
